...
define(function (require, exports, module) {
	var loadIndex;
	var $ = zepto = require('../lib/zepto'), layer = require('./layer'), gconfig = require("./gconfig");
	...
	(function init(){
		document.head.appendChild(function(){
			var path = gconfig.cssPath+gconfig.layerTheme;
		    var link = document.createElement('link');
		    link.href = path+'.css?v='+seajs._sysVersion;
		    link.type = 'text/css';
		    link.rel = 'styleSheet'
		    link.id = 'layermcss';
		    return link;
	 ...
	 */
	function iAlert(alertMsg,title, yesFunc)
	{
		...
		layer.open({ 
			className:(
					function(){
				//alert(window.iBrowser.pc);
				var className = window.iBrowser.pc? "layermchildpc" :  "layermchildmobile"
				return className
					})(),
	        title: title, 
	        content: alertMsg, 
	 ...
	 */
	function iMsg(msgText, msgTime)
	{
		msgTime = msgText?msgText:"提示";
		typeof msgTime == 'number'? msgTime:3;
		msgTime = msgTime==0?3:msgTime;
	    layer.open({ 
	        content: msgText, 
	        time: 3, //2秒后自动关闭 
	    });
	...
	*//**
	 ...
	 */
	function iLoading( isShowOverLay)
	{

		if(loadIndex){//当已有等待页面，则不显示新的
			return;
		}
		isShowOverLay = isShowOverLay?isShowOverLay:false
		loadIndex = layer.open({ 
				        type: 2,
				        shade:true,
				        shadeClose:isShowOverLay
				         //&& config.shadeClose
