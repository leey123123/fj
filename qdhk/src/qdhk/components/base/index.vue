<template>
  <div>
    <header>
    <div class="head"><h1>{{pagetitle}}</h1></div>
...
export default{
    ...
    methods:{
        ...
        submitsuccess:function(){
            ...
            if(!vm.roleFlag){//客户经理登陆
              if(um && typeof um !=undefined){
                sessionStorage.setItem("wx_umAccount",um);
                if(process.env.NODE_ENV !== 'production'){
                  
                  window.location.href="/html/lfs/dist/index.html#/searchClaim";
                }else{
                  window.location.href="/html/lfs/dist/index.html#/searchClaim";
                }
               }else{
    ...
    created:function(){
        var vm = this;
        var bool = this.getData('dataAbled');
        if(bool){
          vm.pagetitle="历史申请";
        }else{
          vm.pagetitle="待办申请";
