...
HttpServer.prototype.handleRequest_ = function(req, res) {
  ...
  for(var i=0; i<rules.length; i++){
    var rule = rules[i],
        reg = new RegExp(rule.location);

    if(reg.test(req.url.path)){
      //支持cookies
      var r = request.defaults({'proxy': rule.proxy_pass, jar: true})
      ...
      if(req.method == 'POST'){
        ...
        req.addListener('data', function(chunk){
         ...
         .addListener('end', function(){
              ...
              //console.log('****************>>>', info);
              r.post('http://localhost:'+DEFAULT_PORT+req.url.path, {form: info}).on('error',function(err){console.log(err);}).pipe(res);
            });
      }else{
        r.get('http://localhost:'+DEFAULT_PORT+req.url.path).on('error',function(err){console.log(err);}).pipe(res);
      }
