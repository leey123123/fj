...
if(argv!==null && argv.length>0){
  ...
  grunt.initConfig({
      ...
      tmp: {
        ...
        options: {
          process: function (content, srcpath) {
            var indexreg = /app\/index\.html/;
            if(srcpath.match(indexreg)){
              var reg = /<script.*main\.js.*script>/;
              //console.log(content.match(reg));
              return content.replace(reg,"<script src=\"main.js\"></script>");
            }
    ...
    uglify: {
      ...
      buildother: {
        options:{
          mangle: {
            except: ['require', 'exports' , 'module','&q','angular','$scope', '$http', '$interval','mainService'] //除了这3个
          },
          preserveComments: false, //删除全部注释，还可以为 all（不删除注释），some（保留@preserve @license @cc_on等注释）
          report: "min" //输出压缩率，可选的值有 false(不输出信息)，gzip
        },
  ...
  grunt.registerTask('aa', ['clean', 'copy:tmp','uglify:buildother']);
  grunt.registerTask('build', ['clean', 'copy:tmp','uglify:buildother','requirejs','clean:baseconf','uglify:buildmain','copy:dist']);
};
}
else{
